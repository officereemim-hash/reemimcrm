...
import {
  ...
  Lightbulb, ChevronDown, Mail, Sparkles,
} from 'lucide-react';
...
const SECTIONS = [
  ...
  { id: 'system-agent', icon: Sparkles, color: 'hsl(270,47%,32%)', title: 'סוכן המערכת רעמים — ניהול מוואטסאפ',
    intro: 'סוכן AI פרטי לצוות בלבד (לא הבוט של הלקוחות). מנהלים איתו את המערכת מהוואטסאפ.',
    points: [
      'חיבור חד-פעמי: הגדרות → כללי → כפתור "התחברות לסוכן בוואטסאפ". נפתחת שיחה — שולחים את ההודעה שמופיעה, וזה הכל. שומרים את המספר באנשי הקשר.',
      'כל בוקר כותבים לסוכן "יש חדש?" והוא מחזיר סיכום: מי ממתין לחזרה, פניות חדשות, פגישות היום ומחר, ומשימות.',
      'אפשר לבקש בשפה חופשית: "עדכני את הפנייה של דנה לפגישה נקבעה", "פתחי משימה לבר להתקשר מחר", "מה הסטטוס של יעקב סבג", "שני את ההודעה של אישור הפגישה".',
      'הודעה קולית — פשוט מקליטים ושולחים; הסוכן מתמלל ומבצע.',
      'קובץ / תמונה / PDF בכל פורמט — שולחים לסוכן וכותבים לאיפה לשמור (מסמך של לקוח, הסכם, תמונת משרד וכו׳).',
      'שליטה בבוט הלקוחות: "כבי את הבוט", "השהי בוט למספר 05X...", "החזירי בוט למספר 05X...", "חסמי את המספר X".',
    ],
    note: 'כשפונה מבקש שיחזרו אליו — נשלח מייל אוטומטי לבשמת (bosmat@oryx-alt.com) עם השם, הטלפון, סוג הפנייה וקישור לפנייה במערכת. אין צורך לעשות דבר כדי להפעיל את זה.' },
