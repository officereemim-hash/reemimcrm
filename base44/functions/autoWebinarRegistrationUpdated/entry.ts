...
Deno.serve(async (req) => {
  try {
    ...
    if (attendedNow) {
      ...
      // נעילת חלון זמן: המסלול מופעל משני טריגרים במקביל (קריאה מ-zoomWebhook + אוטומציית עדכון),
      // ושניהם רצו לפני שהדגל נכתב — לכן בדיקה גם לפי רישום תקשורת מהדקות האחרונות.
